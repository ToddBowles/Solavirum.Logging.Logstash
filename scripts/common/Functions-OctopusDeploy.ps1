function Get-OctopusToolsExecutable
{
    ...
    $commondScriptsDirectoryPath = "$rootDirectoryPath\scripts\common"
...
function Get-LastReleaseToEnvironment
{
    ...
    $version = if ($release -eq $null) 
    {
        Write-Verbose "No deployment was found for project [$projectName] in environment [$environmentName]. Defaulting version to 'latest'."
        return "latest" 
    } 
    else 
    {
        Write-Verbose "Deployment with version [$($release.Version)] was found for project [$projectName] in environment [$environmentName]."
        $release.Version 
    }
